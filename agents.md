...
* **GSAP animations** are handled via `data-gsap` attributes using `gsap-motion.js`.
* **Framer Motion Transition**: Match Figma/React motion designs by using the corresponding `gsap-motion.js` presets to maintain the intended feel.
...
<section class="section-{{ section.id }}">
  <h2 data-gsap="fade-up">{{ section.settings.heading }}</h2>
</section>
...
* Use Tailwind for styling and `data-gsap` for animations.
...
* **Stagger**:
```html
<div data-gsap="stagger-fade-up" data-stagger="0.1">
  <div>Item 1</div>
  <div>Item 2</div>
</div>
```
...
## Goal
Transform React/Figma designs into clean Liquid structures, editable Shopify sections, and Tailwind-based styling with GSAP interactions—avoiding unnecessary JS complexity.
